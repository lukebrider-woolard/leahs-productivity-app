...
import { Box, Button, Fab, FormControl, InputLabel, MenuItem, Modal, OutlinedInput, Select, SelectChangeEvent, Stack, TextField, Tooltip, Typography } from '@mui/material';
...
export default function AddMagnetForm() {
  ...
  const [availableBundles, setAvailableBundles] = useState<string[]>(uniqueBundles);
  ...
  function submitNewBundle() {
    setAvailableBundles(
      [
        ...availableBundles, newBundleName
      ]
    );
    handleModalClose();
  }
  ...
  function renderForm() {
    return (
      <Box component='form' justifyContent='space-evenly' alignItems='center' display='flex' flexWrap='wrap' sx={{ pt: 10 }}>
        <TextField 
          id='magnet-id' 
          label='Magnet ID' 
          variant='outlined' 
        ...
        <FormControl sx={{ width: 300 }}>
          <InputLabel id='select-bundles-label'>
            Bundles 
          </InputLabel>
          <Select
          ...
          <Tooltip title='Add New Bundle'>
            <Fab size='small' color='secondary' aria-label='add' sx={{ position: 'absolute', right: 20, top: 50 }} onClick={() => handleModalOpen()}>
              <AddIcon />
            </Fab>
          </Tooltip>
        </FormControl>
  ...
  return (
    <>
      ...
      <Button variant='text' sx={{ position: 'fixed', top:  80, right: 20 }} onClick={() => navigate('/datapage')}>
